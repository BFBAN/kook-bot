  ...
  func: AppFunc = async (session) => {
    try {
      ...
      if (resSearch.data.length > 0) {
        message = new SearchListTemplate(resSearch.data).generation(other, mainValue, this.expirationTime, other.get("lang") ?? config.i18n.default);
      ...
      if (resSearch.data.length > 1) {
        ...
        bot.on('buttonClick', async event => {
          if (event.targetMsgId == replyId.msgSent.msgId && event.type == "buttonClick") {
            ...
            let player_info = await that.getPlayerInfo(selectItemData.uid);

            if (player_info) {
              clearTimeout(timer); // 关闭超时计时器

              // 发送玩家卡片
              await session.updateMessage(replyId.msgSent.msgId, new PlayerCardTemplate(player_info).generation(other.get("lang") ?? config.i18n.default).toString());
            } else {
